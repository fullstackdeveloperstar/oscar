...
<div class="ballot-header">
    <div id="title" style="box-shadow: 0 3px 4px rgba(0,0,0,0.2);">
		...
		<h2>90th Academy Awards</h2>
		<h1 class="header-title m-t-0 m-b-30">OSCARS BALLOT</h1>										
	...
	<div style="background-color: #f6f6f6;padding-top:13px;padding-bottom:13px;margin-top:-6px;margin-right:0;margin-left:0;" class="row">
		...
		<div class="col-md-6" style="background-color: #f6f6f6;margin-top: -10px;">	
			...
			<h4>Film with the Most Oscars : <input type="text" id="info1" style="height: 20px; border: 0; border-bottom: solid 1px;background-color: #f6f6f6;margin-left:10px;" value="<?php echo $info1;?>" maxlength="20" /></h4>
			<h4>How many? <input type="text" id="info2" style="height: 20px; border: 0; border-bottom: solid 1px;background-color: #f6f6f6;margin-left:10px;" value="<?php echo $info2;?>"  maxlength="20" /></h4>	
		</div>
	</div>
</div>
		...
		<div class="ballot-box ballots-col3">
			...
			<?php 
				for ($j=0; $j<count($problems[$i]['data']); $j++) { 
					...
					echo '<input  type="radio" '.$name.' id="radio'.$value.'" value="'.$value.'" '.$checked.' '.$disabled.'>';
